...
//!   * **`BW(F_{2^m})`** — on the finite nim-field leg the nonsingular quadratic
//!     class is the Arf/Witt class `W_q(F_{2^m}) ≅ ℤ/2`; the Brauer-Wall operation is
//!     XOR, checked against actual direct sums of hyperbolic and anisotropic planes.
//!
...
impl BrauerWallClass {
    ...
    pub fn try_add(&self, other: &BrauerWallClass) -> Result<BrauerWallClass, &'static str> {
        match (*self, *other) {
            ...
            (BrauerWallClass::Char2 { arf: a }, BrauerWallClass::Char2 { arf: b }) => {
                Ok(BrauerWallClass::Char2 { arf: a ^ b })
            }
    ...
    pub fn zero_like(&self) -> BrauerWallClass {
        match *self {
            ...
            BrauerWallClass::Char2 { .. } => BrauerWallClass::Char2 { arf: 0 },
        }
    }
}
...
pub fn bw_class_nimber(metric: &Metric<Nimber>) -> Option<BrauerWallClass> {
    match WittClassG::try_char2_from_metric(metric).ok()? {
        WittClassG::Char2 { arf } => Some(BrauerWallClass::Char2 { arf }),
...
mod tests {
    ...
    fn bw_char2_is_arf_witt_z2_and_homomorphic() {
        ...
        assert_eq!(bw_class_nimber(&h), Some(BrauerWallClass::Char2 { arf: 0 }));
        assert_eq!(bw_class_nimber(&a), Some(BrauerWallClass::Char2 { arf: 1 }));
        assert_eq!(
            bw_class_nimber(&a.direct_sum(&a)),
            Some(BrauerWallClass::Char2 { arf: 0 })
        );
        assert_eq!(
