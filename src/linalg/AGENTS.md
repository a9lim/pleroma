...
- **`field.rs`** — Gaussian solve / `inverse_matrix` / unit-pivot nullspace over a
  `Scalar` field. The crate's generic linear solver: used by the Clifford GA solves
  (`clifford::multivector_inverse`, blade analysis, `inverse_outermorphism`, the
  spinor builder) and the integral-lattice/symplectic/ramified layers.
