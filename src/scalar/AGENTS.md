...
- **`extension.rs`** — the `FieldExtension: Scalar` trait: a finite separable
  ...
  - **`CyclicGaloisExtension: FieldExtension`** adds the Galois data the twisted
    ...
    coordinate basis), `Nimber` (σ=nim-Frobenius, bit basis {1,2,…,2¹²⁷}). The
    relative trace stays `FieldExtension::trace` — σ/basis are the only new data, so
    it's a thin subtrait. `Qq` is left out for now (it has trace/norm already; a
    Teichmüller F-basis of `W_N(F_q)/W_N(F_p)` is a clean follow-up — honest
    exclusion, not a stub). Consumed by `forms/trace_form.rs`.
