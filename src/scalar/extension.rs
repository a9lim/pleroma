...
// are the genuine `Σσⁱ` / `Πσⁱ`. The Witt Frobenius is built exactly from the Witt
// components (`witt_components` ∘ Fpn-Frobenius ∘ `from_witt_components`) — no
// precision loss, since Q_q multiplication is exact.
//
...
/// The Witt Frobenius `σ` on `W_N(F_q)`: raise every Witt component to the `p`-th
/// power. The lift of `x ↦ x^p` on `F_q`, a ring automorphism.
...
/// Witt components in `F_p` ⇒ `.0[1..] = 0`) down to the base `Qq<P,N,1>` (= `Q_p`).
